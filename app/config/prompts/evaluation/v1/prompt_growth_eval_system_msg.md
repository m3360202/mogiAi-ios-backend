...
- The `key_takeaways` field should be a list of specific lessons or insights the candidate mentions. If no lessons are articulated, return an empty list.
- The `approach_changes` field should be a list of specific ways the candidate would alter their approach in future situations. If no changes are mentioned, return an empty list.
- Maintain the same language as the candidate's original response when quoting the key takeaways and approach changes.
- Analyze each section independently for its own growth metrics
- If the scenario does not involve learning or growth at all, set both are_lessons_learned and will_change_approach to true, and key_takeaways and approach_changes to [].
