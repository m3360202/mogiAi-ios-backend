"""
Test file for EvidenceMetricCalculationService.

This file contains tests to verify the evidence metric calculation service works correctly.
...
def test_scoring_logic() -> None:
    ...
    print("  - Poor: No evidence at all should result in POOR score (30.0)")
    print("  - Good: Provides evidences AND evidence is relevant should result in GOOD score (85.0)")  
    print("  - Fair: Evidence present but not specific or not relevant should result in FAIR score (65.0)")
...
def test_service_initialization() -> None:
    ...
    for model in models_to_test:
        service = EvidenceMetricCalculationService(logger, id_generator, model=model)
...
def test_create_metric_group_integration(service: EvidenceMetricCalculationService, dialog_sections: List[DialogSection]) -> None:
    """Test the main create_metric_group method (integration test - may fail without API key)."""
    metadata = MetricMetadata(metric_type=MetricType.EVIDENCE, model="gpt-4o", weight=1.0)
    
    try:
        metric_group = asyncio.run(service.create_metric_group(dialog_sections, metadata))
        
        # Verify metric group structure
        assert metric_group.metric_type == MetricType.EVIDENCE
        ...
        for i, metric in enumerate(metric_group.metrics):
            ...
            assert metric.metadata.metric_type == MetricType.EVIDENCE
        ...
        print("✓ Integration test - MetricGroup created successfully:")
...
def test_sub_metrics_generation(service: EvidenceMetricCalculationService) -> None:
    """Test sub-metrics generation with different dialog scenarios."""
    
    # Test Case 1: Poor evidence (no evidence at all)
    ...
    # Test Case 2: Good evidence (provides evidences, relevant)
    ...
    # Test Case 3: Fair evidence (provides evidences, not relevant)
    ...
    test_cases = [
        ("Poor Evidence", poor_dialog_section),
        ("Good Evidence", good_dialog_section),
        ("Fair Evidence", fair_dialog_section)
    ]
    ...
    for test_name, dialog_section in test_cases:
        try:
            ...
            assert "provides_evidences" in metric.sub_metrics
            assert "main_idea" in metric.sub_metrics
            assert "evidences" in metric.sub_metrics
            assert "relevance_of_evidence" in metric.sub_metrics
            
            # Verify data types
            assert isinstance(metric.sub_metrics["provides_evidences"], bool)
            assert isinstance(metric.sub_metrics["main_idea"], str)
            assert isinstance(metric.sub_metrics["evidences"], list)
            assert isinstance(metric.sub_metrics["relevance_of_evidence"], bool)
            
            print(f"✓ Sub-metrics test - {test_name}:")
            print(f"  - Provides evidences: {metric.sub_metrics['provides_evidences']}")
            print(f"  - Main idea: {metric.sub_metrics['main_idea']}")
            print(f"  - Evidence count: {len(metric.sub_metrics['evidences'])}") # type: ignore
            print(f"  - Relevance of evidence: {metric.sub_metrics['relevance_of_evidence']}")
...
def test_revision_generation(service: EvidenceMetricCalculationService) -> None:
    ...
    poor_dialog_section = DialogSection(
        ...
        messages=[
            DialogMessage(
                ...
                role=MessageRole.INTERVIEWER,
                content="What makes you a strong team leader?",
                start_time=datetime.now(),
                end_time=datetime.now()
            ),
            DialogMessage(
                section_id="test_section_revision_poor",
                role=MessageRole.CANDIDATE,
                content="I'm a great leader and I work well with people.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    good_dialog_section = DialogSection(
        ...
        messages=[
            ...
            DialogMessage(
                section_id="test_section_revision_good",
                role=MessageRole.CANDIDATE,
                content="I believe I'm a strong team leader because I focus on clear communication and empowerment. For example, in my last project, I established weekly one-on-ones with each team member to understand their challenges and goals. I also delegated meaningful responsibilities based on their strengths, which resulted in a 25% increase in team productivity and improved employee satisfaction scores.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    test_cases = [
        ("Poor Evidence", poor_dialog_section, True),  # Should have substantial revision
        ("Good Evidence", good_dialog_section, False)  # Should have minimal/no revision
    ]
...
def test_evidence_service() -> None:
    """Test the evidence metric calculation service."""
    ...
    dialog_sections = [DialogSection(
        ...
        messages=[
            DialogMessage(
                ...
                role=MessageRole.INTERVIEWER,
                content="Tell me about a time you improved a process at work.",
                start_time=datetime.now(),
                end_time=datetime.now()
            ),
            DialogMessage(
                section_id="test_section_1",
                role=MessageRole.CANDIDATE,
                content="I identified that our code review process was taking too long. I analyzed our metrics and found that reviews were averaging 3 days. I implemented a new workflow with automated checks and designated reviewers by expertise area. This reduced review time to 1 day and improved code quality scores by 15%.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    try:
        ...
        service = EvidenceMetricCalculationService(logger, id_generator, model="gpt-4o")
        print("✓ Successfully created EvidenceMetricCalculationService")
...
def test_experimental_prompts() -> None:
    ...
    custom_eval_system_prompt_path = "/Users/wanghuan/Documents/py_repos/career_face_backend/app/config/prompts/evaluation/v2/prompt_evidence_eval_system_msg.md"  # e.g., "/path/to/custom_eval_system.md"
    ...
    try:
        ...
        metadata = MetricMetadata(metric_type=MetricType.EVIDENCE, model=model, weight=1.0)
        metric_group = asyncio.run(service.create_metric_group(dialog_sections, metadata))
