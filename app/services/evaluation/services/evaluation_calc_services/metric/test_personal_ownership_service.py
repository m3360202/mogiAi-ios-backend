"""
Test file for PersonalOwnershipMetricCalculationService.

This file contains tests to verify the personal ownership metric calculation service works correctly.
...
def test_scoring_logic() -> None:
    ...
    print("  - Good: Has personal contributions with specific examples should result in GOOD score (3.0)")
    print("  - Poor: No personal contributions mentioned should result in POOR score (1.0)")
...
def test_service_initialization() -> None:
    ...
    for model in models_to_test:
        service = PersonalOwnershipMetricCalculationService(logger, id_generator, model=model)
...
def test_create_metric_group_integration(service: PersonalOwnershipMetricCalculationService, dialog_sections: List[DialogSection]) -> None:
    """Test the main create_metric_group method (integration test - may fail without API key)."""
    metadata = MetricMetadata(metric_type=MetricType.PERSONAL_OWNERSHIP, model="gpt-4o", weight=1.0)
    
    try:
        metric_group = asyncio.run(service.create_metric_group(dialog_sections, metadata))
        
        # Verify metric group structure
        assert metric_group.metric_type == MetricType.PERSONAL_OWNERSHIP
        ...
        for i, metric in enumerate(metric_group.metrics):
            ...
            assert metric.metadata.metric_type == MetricType.PERSONAL_OWNERSHIP
...
def test_sub_metrics_generation(service: PersonalOwnershipMetricCalculationService) -> None:
    """Test sub-metrics generation with different dialog scenarios."""
    
    # Test Case 1: Poor personal ownership (focuses on team efforts)
    ...
    # Test Case 2: Good personal ownership (clear personal contributions)
    good_dialog_section = DialogSection(
        ...
        messages=[
            ...
            DialogMessage(
                section_id="test_section_good",
                role=MessageRole.CANDIDATE,
                content="I identified the root cause of a recurring issue in our system and implemented a fix that reduced downtime by 30%. I also documented the solution and trained the team on the new process.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    test_cases = [
        ("Poor Personal Ownership", poor_dialog_section),
        ("Good Personal Ownership", good_dialog_section)
    ]
    ...
    for test_name, dialog_section in test_cases:
        try:
            ...
            assert "has_personal_contribution" in metric.sub_metrics
            assert "personal_contributions" in metric.sub_metrics
            
            # Verify data types
            assert isinstance(metric.sub_metrics["has_personal_contribution"], bool)
            assert isinstance(metric.sub_metrics["personal_contributions"], list)
            
            print(f"✓ Sub-metrics test - {test_name}:")
            print(f"  - Has personal contribution: {metric.sub_metrics['has_personal_contribution']}")
            print(f"  - Personal contributions count: {len(metric.sub_metrics['personal_contributions'])}") # type: ignore
            print(f"  - Score: {metric.score.score_label.value}")
            
            if metric.sub_metrics["personal_contributions"]:
                print(f"  - Personal contributions: {metric.sub_metrics['personal_contributions']}")
            
        except (ValueError, ConnectionError, RuntimeError) as e:
            print(f"⚠ Sub-metrics test skipped for {test_name} - API issue: {e}")


def test_revision_generation(service: PersonalOwnershipMetricCalculationService) -> None:
    ...
    test_cases = [
        ("Poor Personal Ownership", poor_dialog_section, True),  # Should have substantial revision
        ("Good Personal Ownership", good_dialog_section, False)  # Should have no revision
    ]
...
def test_personal_ownership_service() -> None:
    """Test the personal ownership metric calculation service."""
        ...
        DialogSection(
            ...
            messages=[
                DialogMessage(
                    ...
                    role=MessageRole.INTERVIEWER,
                    content="Tell me about a time you led a team through a difficult situation.",
                    start_time=datetime.now(),
                    end_time=datetime.now()
                ),
                DialogMessage(
                    section_id="test_section_1",
                    role=MessageRole.CANDIDATE,
                    content="I took charge when our project was behind schedule. I analyzed the bottlenecks, reassigned tasks based on team strengths, and implemented daily stand-ups to track progress. I also communicated with stakeholders to manage expectations. As a result, I delivered the project on time and improved team morale.",
                    start_time=datetime.now(),
                    end_time=datetime.now()
                )
            ],
        ...
        DialogSection(
            ...
            messages=[
                DialogMessage(
                    ...
                    role=MessageRole.INTERVIEWER,
                    content="How do you approach collaboration?",
                    start_time=datetime.now(),
                    end_time=datetime.now()
                ),
                DialogMessage(
                    section_id="test_section_2",
                    role=MessageRole.CANDIDATE,
                    content="We usually work together as a team and everyone contributes to the final outcome.",
                    start_time=datetime.now(),
                    end_time=datetime.now()
                )
            ],
    ...
    try:
        ...
        service = PersonalOwnershipMetricCalculationService(logger, id_generator, model="gpt-4o")
        print("✓ Successfully created PersonalOwnershipMetricCalculationService")
...
def test_experimental_prompts() -> None:
    ...
    custom_eval_system_prompt_path = "/Users/wanghuan/Documents/py_repos/career_face_backend/app/config/prompts/evaluation/v2/prompt_personal_ownership_eval_system_msg.md"  # e.g., "/path/to/custom_eval_system.md"
    ...
    try:
        ...
        metadata = MetricMetadata(metric_type=MetricType.PERSONAL_OWNERSHIP, model=model, weight=1.0)
        metric_group = asyncio.run(service.create_metric_group(dialog_sections, metadata))
...
if __name__ == "__main__":
    test_personal_ownership_service()