"""
Growth Metric Calculation Service.

This service implements the growth metric calculation based on two criteria:
1. Lesson Learned - Were lessons or key takeaways articulated by the candidate?
2. Approaches Change to Future Situations - Does the candidate describe how they would change their approach in future similar situations based on what they learned?
...
class GrowthSubMetrics(BaseModel):
    """
    Internal data structure for growth sub-metrics.
    ...
    are_lessons_learned: bool
    key_takeaways: List[str] = []
    will_change_approach: bool
    approach_changes: List[str] = []
    
    model_config = {"frozen": True}


class GrowthMetricCalculationService(MetricCalculationService):
    """
    Growth metric calculation service implementation.
    
    Evaluates candidate responses based on:
    - Lessons learned
    - Approach changes for future situations
    """
    ...
    async def create_metric_group(
        ...
        """
        Create a complete MetricGroup entity for growth evaluation.
        ...
        """
        try:
            self.logger.info("Creating growth metric group", {
            ...
            for i, dialog_section in enumerate(dialog_sections):
                ...
                self.logger.info("Processing dialog section result for growth", {
                ...
                self.logger.info("Successfully created growth metric for section", {
                    "dialog_section_id": dialog_section.id,
                    "section_index": dialog_section.section_index,
                    "metric_id": metric_id,
            ...
            self.logger.info("Successfully created growth metric group", {
                "dialog_sections_count": len(dialog_sections),
            ...
            self.logger.error("Failed to create growth metric group", e, {
                "dialog_sections_count": len(dialog_sections),
                "service": "GrowthMetricCalculationService"
            })
            raise
    ...
    def _load_prompts(self) -> None:
        ...
        try:
            ...
            if self.custom_eval_system_prompt_path:
                ...
                eval_system_path = default_prompt_dir / "prompt_growth_eval_system_msg.md"
            ...
            if self.custom_revise_system_prompt_path:
                ...
                revise_system_path = default_prompt_dir / "prompt_growth_revise_system_msg.md"
            ...
            self.logger.info("Growth prompts loaded successfully", {
                "service": "GrowthMetricCalculationService",
                "custom_prompts": {
            ...
            self.logger.error("Failed to load growth prompts", e, {
                "service": "GrowthMetricCalculationService"
            })
            raise
    
    async def _generate_sub_metrics_for_all_sections(self, dialog_sections: List[DialogSection]) -> List[GrowthSubMetrics]:
        """
        ...
        Returns:
            List[GrowthSubMetrics]: List containing evaluation results for each section
        """
        try:
            self.logger.info("Generating growth sub-metrics for all sections", {
                "dialog_sections_count": len(dialog_sections),
                "service": "GrowthMetricCalculationService"
            })
            ...
            for dialog_section in dialog_sections:
                ...
                for msg in dialog_section.messages:
                    # 安全处理时间戳（避免 OSError）
                    try:
                        start_ts = msg.start_time.timestamp() if hasattr(msg.start_time, 'timestamp') else float(msg.start_time)
                        end_ts = msg.end_time.timestamp() if hasattr(msg.end_time, 'timestamp') else float(msg.end_time)
            ...
            for i, section_result in enumerate(result_list):
                ...
                sub_metrics = GrowthSubMetrics.model_validate(section_result)
                section_results.append(sub_metrics)
            
            self.logger.info("Successfully generated growth sub-metrics for all sections", {
                "dialog_sections_count": len(dialog_sections),
                "service": "GrowthMetricCalculationService"
            })
            ...
            self.logger.error("Failed to generate growth sub-metrics for all sections", e, {
                "dialog_sections_count": len(dialog_sections),
                "service": "GrowthMetricCalculationService"
            })
            raise
    
    async def _generate_revisions_for_all_sections(self, sections_needing_revision: List[Tuple[int, DialogSection, GrowthSubMetrics, Score]]) -> List[str]:
        """
        ...
        try:
            self.logger.info("Generating growth revisions for all sections", {
            ...
            self.logger.info("Successfully generated growth revisions for all sections", {
                "sections_count": len(sections_needing_revision),
            ...
            self.logger.error("Failed to generate growth revisions for all sections", e, {
                "sections_count": len(sections_needing_revision),
                "service": "GrowthMetricCalculationService"
            })
            raise
    
    async def _generate_sub_metrics(self, dialog_section: DialogSection) -> GrowthSubMetrics:
        """
        Generate sub-metrics for growth evaluation using LLM API.
        
        Args:
            dialog_section: The dialog section to analyze
            
        Returns:
            GrowthSubMetrics containing evaluation results
        """
        try:
            self.logger.info("Generating growth sub-metrics", {
                "dialog_section_id": dialog_section.id,
                "service": "GrowthMetricCalculationService"
            })
            ...
            self.logger.info("Successfully generated growth sub-metrics", {
                "dialog_section_id": dialog_section.id,
                "service": "GrowthMetricCalculationService",
                "are_lessons_learned": sub_metrics.are_lessons_learned,
                "will_change_approach": sub_metrics.will_change_approach,
                "key_takeaways_count": len(sub_metrics.key_takeaways),
                "approach_changes_count": len(sub_metrics.approach_changes)
            })
            
            return sub_metrics
            
        except Exception as e:
            self.logger.error("Failed to generate growth sub-metrics", e, {
                "dialog_section_id": dialog_section.id,
                "service": "GrowthMetricCalculationService"
            })
            raise
    
    def _calculate_score(self, sub_metrics: GrowthSubMetrics) -> Score:
        """
        Calculate growth score based on sub-metrics.
        
        Scoring logic:
        - Good: All metrics pass (lessons learned AND approach changes)
        - Fair: Lessons learned present but no approach changes
        - Poor: No lessons learned and no approach changes
        
        Args:
            sub_metrics: GrowthSubMetrics from _generate_sub_metrics
            
        Returns:
            Score object with label and numeric value
        """
        try:
            if sub_metrics.are_lessons_learned and sub_metrics.will_change_approach:
                # All metrics pass - Good
                score_label = ScoreLabel.GOOD
            elif sub_metrics.are_lessons_learned and not sub_metrics.will_change_approach:
                # Lessons learned but no approach changes - Fair
                score_label = ScoreLabel.FAIR
            else:
                # No lessons learned and no approach changes - Poor
                score_label = ScoreLabel.POOR
            
            score = self.score_transformation_service.create_score(score_label)
            
            self.logger.info("Calculated growth score", {
                "service": "GrowthMetricCalculationService",
            ...
            self.logger.error("Failed to calculate growth score", e, {
                "service": "GrowthMetricCalculationService"
            })
            raise
    
    async def _generate_revision(self, sub_metrics: GrowthSubMetrics, score: Score, dialog_section: DialogSection) -> str:
        """
        Generate revision text for growth improvement using LLM API.
        ...
        """
        try:
            # Good scores don't need revision
            if score.score_label == ScoreLabel.GOOD:
                self.logger.info("No revision needed for good growth score", {
                    "service": "GrowthMetricCalculationService",
                    "score_label": score.score_label.value
                })
                return ""
            
            self.logger.info("Generating growth revision", {
                "service": "GrowthMetricCalculationService",
                "score_label": score.score_label.value
            })
            ...
            self.logger.info("Successfully generated growth revision", {
                "service": "GrowthMetricCalculationService",
                "revision_length": len(revision) if revision else 0
            })
            
            return revision
            
        except Exception as e:
            self.logger.error("Failed to generate growth revision", e, {
                "service": "GrowthMetricCalculationService"
            })
            raise
    
    def _prepare_evaluation_data(self, sub_metrics: GrowthSubMetrics, score: Score) -> Dict[str, Any]:
    ...
    def _convert_sub_metrics_to_dict(self, sub_metrics: GrowthSubMetrics) -> Dict[str, Any]:
        """Convert internal GrowthSubMetrics to dictionary format for Metric entity."""
    ...
    async def _call_llm_with_json_response(self, messages: List[Dict[str, str]]) -> Any:
        ...
        """
        try:
            # Using LiteLLM to call the model
            ...
            self.logger.error("Failed to call LLM or parse JSON response", e, {
                "service": "GrowthMetricCalculationService",
                "model": self.model
            })
            raise