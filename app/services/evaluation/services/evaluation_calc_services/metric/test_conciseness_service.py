"""
Test file for ConcisenessMetricCalculationService.

This file contains tests to verify the conciseness metric calculation service works correctly.
...
def test_scoring_logic() -> None:
    ...
    # Test Case 1: Poor scoring scenario - verify structure
    print("✓ Scoring logic test - Testing expected score calculation behavior")
    print("  - Poor: Core idea not presented should result in POOR score (30.0)")
    print("  - Good: All criteria pass should result in GOOD score (85.0)")  
    print("  - Fair: Core idea present but other criteria fail should result in FAIR score (65.0)")
...
def test_service_initialization() -> None:
    ...
    for model in models_to_test:
        service = ConcisenessMetricCalculationService(logger, id_generator, model=model)
...
def test_create_metric_group_integration(service: ConcisenessMetricCalculationService, dialog_sections: List[DialogSection]) -> None:
    """Test the main create_metric_group method (integration test - may fail without API key)."""
    metadata = MetricMetadata(metric_type=MetricType.CONCISENESS, model="gpt-4o", weight=1.0)
    
    try:
        metric_group = asyncio.run(service.create_metric_group(dialog_sections, metadata))
        
        # Verify metric group structure
        assert metric_group.metric_type == MetricType.CONCISENESS
        ...
        for i, metric in enumerate(metric_group.metrics):
            ...
            assert metric.metadata.metric_type == MetricType.CONCISENESS
...
def test_sub_metrics_generation(service: ConcisenessMetricCalculationService) -> None:
    """Test sub-metrics generation with different dialog scenarios."""
    
    # Test Case 1: Poor conciseness (lots of filler words, no clear core idea)
    poor_dialog_section = DialogSection(
        ...
        messages=[
            DialogMessage(
                ...
                content="What are your leadership strengths?",
            ...
            DialogMessage(
                ...
                content="Well, um, you know, I think that I'm like, basically good at, you know, sort of leading people and stuff. I mean, to be honest, I guess I have some experience with, like, managing teams and things like that.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    # Test Case 2: Good conciseness (clear, direct response)
    good_dialog_section = DialogSection(
        ...
        messages=[
            ...
            DialogMessage(
                section_id="test_section_good",
                role=MessageRole.CANDIDATE,
                content="I excel at strategic planning and team motivation. I consistently deliver projects on time by establishing clear goals and maintaining open communication with stakeholders.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    for test_name, dialog_section in test_cases:
        try:
            ...
            assert "is_core_idea_presented" in metric.sub_metrics
            assert "core_idea" in metric.sub_metrics
            assert "filter_words" in metric.sub_metrics
            assert "strong_words" in metric.sub_metrics
            assert "sentence_count" in metric.sub_metrics
            assert "filter_words_count" in metric.sub_metrics
            assert "strong_words_count" in metric.sub_metrics
            assert "filter_words_frequency" in metric.sub_metrics
            assert "strong_words_frequency" in metric.sub_metrics
            
            # Verify data types
            assert isinstance(metric.sub_metrics["is_core_idea_presented"], bool)
            assert isinstance(metric.sub_metrics["core_idea"], str)
            assert isinstance(metric.sub_metrics["filter_words"], list)
            assert isinstance(metric.sub_metrics["strong_words"], list)
            assert isinstance(metric.sub_metrics["sentence_count"], int)
            assert isinstance(metric.sub_metrics["filter_words_count"], int)
            assert isinstance(metric.sub_metrics["strong_words_count"], int)
            assert isinstance(metric.sub_metrics["filter_words_frequency"], (int, float))
            assert isinstance(metric.sub_metrics["strong_words_frequency"], (int, float))
            
            print(f"✓ Sub-metrics test - {test_name}:")
            print(f"  - Core idea presented: {metric.sub_metrics['is_core_idea_presented']}")
            print(f"  - Filter words count: {metric.sub_metrics['filter_words_count']}")
            print(f"  - Strong words count: {metric.sub_metrics['strong_words_count']}")
...
def test_revision_generation(service: ConcisenessMetricCalculationService) -> None:
    ...
    poor_dialog_section = DialogSection(
        ...
        messages=[
            DialogMessage(
                ...
                role=MessageRole.INTERVIEWER,
                content="Describe your problem-solving approach.",
                start_time=datetime.now(),
                end_time=datetime.now()
            ),
            DialogMessage(
                section_id="test_section_revision_poor",
                role=MessageRole.CANDIDATE,
                content="Well, you know, I think that when I encounter problems, I sort of, like, try to figure things out. I mean, I guess I usually look at different options and stuff, and then I kind of make a decision.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
    ...
    good_dialog_section = DialogSection(
        ...
        messages=[
            ...
            DialogMessage(
                section_id="test_section_revision_good",
                role=MessageRole.CANDIDATE,
                content="I follow a structured approach: analyze the problem, research solutions, evaluate alternatives, implement the best option, and monitor results.",
                start_time=datetime.now(),
                end_time=datetime.now()
            )
        ],
...
def test_conciseness_service() -> None:
    """Test the conciseness metric calculation service."""
        ...
        DialogSection(
            ...
            messages=[
                DialogMessage(
                    ...
                    role=MessageRole.INTERVIEWER,
                    content="What's your leadership style?",
                    start_time=datetime.now(),
                    end_time=datetime.now()
                ),
                DialogMessage(
                    section_id="test_section_2",
                    role=MessageRole.CANDIDATE,
                    content="I focus on collaborative leadership. I establish clear objectives, delegate appropriately, and maintain regular communication to ensure team alignment and project success.",
                    start_time=datetime.now(),
                    end_time=datetime.now()
                )
            ],
    ...
    try:
        ...
        service = ConcisenessMetricCalculationService(logger, id_generator, model="gpt-4o")
        print("✓ Successfully created ConcisenessMetricCalculationService")
...
def test_experimental_prompts() -> None:
    ...
    custom_eval_system_prompt_path = "/Users/wanghuan/Documents/py_repos/career_face_backend/app/config/prompts/evaluation/v2/prompt_conciseness_eval_system_msg.md"  # e.g., "/path/to/custom_eval_system.md"
    ...
    try:
        ...
        metadata = MetricMetadata(metric_type=MetricType.CONCISENESS, model=model, weight=1.0)
        metric_group = asyncio.run(service.create_metric_group(dialog_sections, metadata))
