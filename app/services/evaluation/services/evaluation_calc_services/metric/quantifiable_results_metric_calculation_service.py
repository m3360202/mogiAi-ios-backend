"""
Quantifiable Results Metric Calculation Service.

This service implements the quantifiable results metric calculation based on two criteria:
1. Has Results - Does the candidate's act has results?
2. Quantifiable Results - Are the results backed by specific data or numbers?
...
class QuantifiableResultsSubMetrics(BaseModel):
    """
    Internal data structure for quantifiable results sub-metrics.
    ...
    has_results: bool
    results: List[str] = []
    quantifiable_results: bool
    
    model_config = {"frozen": True}


class QuantifiableResultsMetricCalculationService(MetricCalculationService):
    """
    Quantifiable results metric calculation service implementation.
    ...
    async def create_metric_group(
        ...
        """
        Create a complete MetricGroup entity for quantifiable results evaluation.
        ...
        """
        try:
            self.logger.info("Creating quantifiable results metric group", {
            ...
            for i, dialog_section in enumerate(dialog_sections):
                ...
                self.logger.info("Processing dialog section result for quantifiable results", {
                ...
                self.logger.info("Successfully created quantifiable results metric for section", {
                    "dialog_section_id": dialog_section.id,
                    "section_index": dialog_section.section_index,
                    "metric_id": metric_id,
            ...
            self.logger.info("Successfully created quantifiable results metric group", {
                "dialog_sections_count": len(dialog_sections),
            ...
            self.logger.error("Failed to create quantifiable results metric group", e, {
                "dialog_sections_count": len(dialog_sections),
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    ...
    def _load_prompts(self) -> None:
        ...
        try:
            ...
            if self.custom_eval_system_prompt_path:
                ...
                eval_system_path = default_prompt_dir / "prompt_quantifiable_results_eval_system_msg.md"
            ...
            if self.custom_revise_system_prompt_path:
                ...
                revise_system_path = default_prompt_dir / "prompt_quantifiable_results_revise_system_msg.md"
            ...
            self.logger.info("Quantifiable results prompts loaded successfully", {
                "service": "QuantifiableResultsMetricCalculationService",
                "custom_prompts": {
            ...
            self.logger.error("Failed to load quantifiable results prompts", e, {
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    
    async def _generate_sub_metrics_for_all_sections(self, dialog_sections: List[DialogSection]) -> List[QuantifiableResultsSubMetrics]:
        """
        ...
        Returns:
            List[QuantifiableResultsSubMetrics]: List containing evaluation results for each section
        """
        try:
            self.logger.info("Generating quantifiable results sub-metrics for all sections", {
                "dialog_sections_count": len(dialog_sections),
                "service": "QuantifiableResultsMetricCalculationService"
            })
            ...
            for i, section_result in enumerate(result_list):
                ...
                sub_metrics = QuantifiableResultsSubMetrics.model_validate(section_result)
                section_results.append(sub_metrics)
            
            self.logger.info("Successfully generated quantifiable results sub-metrics for all sections", {
                "dialog_sections_count": len(dialog_sections),
                "service": "QuantifiableResultsMetricCalculationService"
            })
            ...
            self.logger.error("Failed to generate quantifiable results sub-metrics for all sections", e, {
                "dialog_sections_count": len(dialog_sections),
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    ...
    async def _generate_revisions_for_all_sections(self, sections_needing_revision: List[tuple]) -> List[str]:
        ...
        try:
            self.logger.info("Generating quantifiable results revisions for all sections", {
            ...
            self.logger.info("Successfully generated quantifiable results revisions for all sections", {
                "sections_count": len(sections_needing_revision),
            ...
            self.logger.error("Failed to generate quantifiable results revisions for all sections", e, {
                "sections_count": len(sections_needing_revision),
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    
    async def _generate_sub_metrics(self, dialog_section: DialogSection) -> QuantifiableResultsSubMetrics:
        """
        Generate sub-metrics for quantifiable results evaluation using LLM API.
        
        Args:
            dialog_section: The dialog section to analyze
            
        Returns:
            QuantifiableResultsSubMetrics containing evaluation results
        """
        try:
            self.logger.info("Generating quantifiable results sub-metrics", {
                "dialog_section_id": dialog_section.id,
                "service": "QuantifiableResultsMetricCalculationService"
            })
            ...
            # Handle new array format response
            if isinstance(result, dict) and "results" in result and isinstance(result["results"], list):
                # Expected case: LLM returned {"results": [array]} for single section
                result_list = result["results"]
                if len(result_list) >= 1:
                    section_result = result_list[0]
                else:
                    raise ValueError("Expected at least 1 section result in array")
            else:
                # Legacy fallback: LLM returned single object (should not happen with new prompts)
                section_result = result
            
            # Use Pydantic's model_validate to parse LLM response directly
            sub_metrics = QuantifiableResultsSubMetrics.model_validate(section_result)
            
            self.logger.info("Successfully generated quantifiable results sub-metrics", {
                "dialog_section_id": dialog_section.id,
                "service": "QuantifiableResultsMetricCalculationService",
                "has_results": sub_metrics.has_results,
                "quantifiable_results": sub_metrics.quantifiable_results,
                "results_count": len(sub_metrics.results)
            })
            
            return sub_metrics
            
        except Exception as e:
            self.logger.error("Failed to generate quantifiable results sub-metrics", e, {
                "dialog_section_id": dialog_section.id,
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    
    def _calculate_score(self, sub_metrics: QuantifiableResultsSubMetrics) -> Score:
        """
        Calculate quantifiable results score based on sub-metrics.
        
        Scoring logic:
        - Good: All metrics pass (has results AND quantifiable results)
        - Fair: Results present but not quantifiable
        - Poor: Neither results nor quantifiable data present
        
        Args:
            sub_metrics: QuantifiableResultsSubMetrics from _generate_sub_metrics
            
        Returns:
            Score object with label and numeric value
        """
        try:
            self.logger.info("Calculating quantifiable results score", {
                "has_results": sub_metrics.has_results,
                "quantifiable_results": sub_metrics.quantifiable_results,
                "service": "QuantifiableResultsMetricCalculationService"
            })
            ...
            if sub_metrics.has_results and sub_metrics.quantifiable_results:
                score_label = ScoreLabel.GOOD
            elif sub_metrics.has_results and not sub_metrics.quantifiable_results:
            ...
            self.logger.info("Successfully calculated quantifiable results score", {
            ...
            self.logger.error("Failed to calculate quantifiable results score", e, {
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    
    async def _generate_revision(self, sub_metrics: QuantifiableResultsSubMetrics, score: Score, dialog_section: DialogSection) -> str:
        """
        Generate revision text for quantifiable results improvement using LLM API.
        ...
        """
        try:
            self.logger.info("Generating quantifiable results revision", {
                "dialog_section_id": dialog_section.id,
                "score_label": score.score_label.value,
                "service": "QuantifiableResultsMetricCalculationService"
            })
            ...
            if isinstance(result, dict) and "results" in result and isinstance(result["results"], list):
                # Expected case: LLM returned {"results": [array]} for single section
                result_list = result["results"]
                if len(result_list) >= 1 and isinstance(result_list[0], dict):
                    revision = result_list[0].get("revised_speech", "") or ""
                else:
                    revision = ""
            else:
                # Legacy fallback: LLM returned single object (should not happen with new prompts)
            ...
            self.logger.info("Successfully generated quantifiable results revision", {
                "dialog_section_id": dialog_section.id,
            ...
            self.logger.error("Failed to generate quantifiable results revision", e, {
                "dialog_section_id": dialog_section.id,
                "service": "QuantifiableResultsMetricCalculationService"
            })
            raise
    
    def _prepare_evaluation_data(self, sub_metrics: QuantifiableResultsSubMetrics, score: Score) -> Dict[str, Any]:
        """Prepare evaluation data for revision prompt."""
    ...
    def _convert_sub_metrics_to_dict(self, sub_metrics: QuantifiableResultsSubMetrics) -> Dict[str, Any]:
        """Convert internal QuantifiableResultsSubMetrics to dictionary format for Metric entity."""
